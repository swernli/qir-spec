...
populates a result identified by the `ptr` type. The available measurement
functions are defined by the executing backend as part of the QIS. Unlike the
Base Profile, the Adaptive Profile relieves a restriction that qubits can only be
measured at the end of the program and that no quantum operations can be performed
after or conditionally on a measurement outcome.
...
loop_body:                          ; preds = %loop_body, %entry
  ...
  call void @__quantum__qis__cnot__body(ptr null, ptr nonnull inttoptr (i64 %0 to ptr))
  %1 = add i64 %0, 1
  %2 = icmp sle i64 %1, 4
...
error:
  ...
  ret i64 1
exit:
  call void @__quantum__rt__result_record_output(ptr null, getelementptr inbounds ([2 x i8], ptr @0, i32 0, i32 0))
  ret i64 0
}
```
...
exit:
  call void @__quantum__rt__result_record_output(ptr nonnull inttoptr (i64 4 to ptr), getelementptr inbounds ([5 x i8], ptr @0, i32 0, i32 0))
  call void @__quantum__rt__result_record_output(ptr nonnull inttoptr (i64 5 to ptr), getelementptr inbounds ([5 x i8], ptr @1, i32 0, i32 0))
  ret i64 0
}
...
schemas, the `ptr` argument serves as a label that permits the compiler or tool
